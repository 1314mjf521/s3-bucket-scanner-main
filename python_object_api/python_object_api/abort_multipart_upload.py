...
def abort_multipart_upload(bucket_name):
    ...
    try:
        ...
        if response.get("Uploads"):
            for upload in response["Uploads"]:
                ...
                res = s3client.abort_multipart_upload(Bucket=bucket_name, Key=key,UploadId=upload_id)
                print("======== Abort multipart upload Bucket: {} Key: {} UploadId: {} ================".format(bucket_name, key, upload_id))
        else:
...
if __name__ == "__main__":
    bucket_name = "aa2"
    abort_multipart_upload(bucket_name)
